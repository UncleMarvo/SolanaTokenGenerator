import { FC, useEffect, useState } from "react";
import { useRouter } from "next/router";
import { useWallet } from "@solana/wallet-adapter-react";
import LiquidityStep from "../../../../components/flow/LiquidityStep";

/**
 * Step 3: Liquidity Setup Page
 * Allows users to add liquidity to enable trading
 */
const LiquidityPage: FC = () => {
  const router = useRouter();
  const { tokenId } = router.query;
  const { publicKey } = useWallet();
  const [tokenData, setTokenData] = useState<{
    name: string;
    symbol: string;
  } | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    if (router.isReady && tokenId) {
      // Fetch token data from the database or API
      fetchTokenData(tokenId as string);
    }
  }, [router.isReady, tokenId]);

  const fetchTokenData = async (mintAddress: string) => {
    try {
      // For now, we'll use placeholder data
      // In a real implementation, you'd fetch from your database
      setTokenData({
        name: "My Token", // This should come from your database
        symbol: "TOKEN", // This should come from your database
      });
    } catch (error) {
      console.error("Failed to fetch token data:", error);
    } finally {
      setIsLoading(false);
    }
  };

  if (isLoading || !tokenData) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4" />
          <p className="text-muted">Loading token data...</p>
        </div>
      </div>
    );
  }

  if (!publicKey) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-fg mb-4">Wallet Required</h1>
          <p className="text-muted">Please connect your wallet to continue.</p>
        </div>
      </div>
    );
  }

  return (
    <LiquidityStep
      tokenMintAddress={tokenId as string}
      tokenName={tokenData.name}
      tokenSymbol={tokenData.symbol}
    />
  );
};

export default LiquidityPage;

...
import { LpChips, formatLpUsd, getRangeDisplay, getHonestDisplay, getLastTxElement } from "../../lib/lpStats";
...
const TokenSharePage: FC = () => {
  ...
  const [walletLP, setWalletLP] = useState<{ has: boolean; positionsCount: number } | null>(null);
  ...
  const fetchWalletLP = async () => {
    ...
    try {
      const response = await fetch(`/api/token/wallet-lp?mint=${mint}&owner=${walletAddress}&t=${Date.now()}`);
      if (response.ok) {
  ...
  const fetchLpChips = async () => {
    ...
    setIsLpLoading(true);
    try {
      const response = await fetch(`/api/token/lp?mint=${mint}&t=${Date.now()}`);
      if (response.ok) {
        const data = await response.json();
        setLpChips(data);
      }
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-4">
                  {!walletAddress ? (
                   ...
                   >
                     <AiOutlineReload size={16} className={isLpLoading || isWalletLPLoading ? "animate-spin" : ""} />
                     <span>{isLpLoading || isWalletLPLoading ? "Refreshing..." : "Refresh"}</span>
                   </button>
                       ...
                       <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                         ...
                         <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                           autoRefresh ? 'translate-x-6' : 'translate-x-1'
                                           ...
                                           <span className="text-sm text-muted font-medium">Auto-refresh (30s)</span>
                  </label>
                </div>
                 ...
                 <div className="bg-muted/10 border border-muted/20 rounded-lg p-4 text-center">
                   ...
                   <div className="font-semibold text-lg">
                     {!walletAddress ? (
                       ...
                       <span className={walletLP.has ? "text-success" : "text-muted"}>
                         {walletLP.has ? "Yes" : "No"}
                       </span>
                     ) : (
                       <span 
                   ...
                   {walletLP?.positionsCount > 0 && (
                     <div className="text-xs text-muted mt-1">
                       {walletLP.positionsCount} position{walletLP.positionsCount !== 1 ? 's' : ''}
                     </div>
                   )}
                 </div>
              </div>
            ...
            <div className="bg-bg/40 backdrop-blur-2xl rounded-2xl p-8 border border-muted/10 mb-4">
              <details className="group">
                <summary className="cursor-pointer list-none">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold text-muted group-open:text-fg transition-colors">Proof</h2>
                   ...
                   {!isOnChainVerified && !lpChips?.lastTx && !walletLP?.has ? (
                     <div className="text-center py-8">
                       ...
                       <h3 className="text-lg font-semibold text-muted mb-2">No proofs yet</h3>
                       <p className="text-sm text-muted mb-4">
                         Add liquidity or enforce Honest Launch to generate proof data.
                       </p>
                           ...
                           onClick={async () => {
                             try {
                               ...
                               if (response.ok) {
                                 ...
                                 if (result.verified) {
                                   alert("✅ Token verified as honest on-chain!");
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h3 className="font-semibold">Honest Verification</h3>
                          <p className="text-sm text-muted">
                            {isOnChainVerified ? "Verified on-chain" : "Not yet verified"}
                          </p>
                        </div>
                      </div>
                      <button
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted">
                              {walletLP.positionsCount} position{walletLP.positionsCount !== 1 ? 's' : ''} found
                            </p>
                          </div>
                        </div>
                        <a
                          href={`/positions?filter=${encodeURIComponent(token.mintAddress)}`}
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          proofText += `✅ Honest: ${isOnChainVerified ? "Verified" : "Not verified"}\n`;
                          ...
                          if (walletLP?.has) {
                            proofText += `👛 Your LP: Yes (${walletLP.positionsCount} position${walletLP.positionsCount !== 1 ? 's' : ''})\n`;
                          }
             ...
             <div className="bg-bg/40 backdrop-blur-2xl rounded-2xl p-8 border border-muted/10 mb-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                 ...
                 <button
                   onClick={() => {
                     const shareUrls = getSocialShareUrls(window.location.href, token.name, token.symbol);
               ...
               <div className="mt-6 p-4 bg-muted/10 rounded-lg border border-muted/20">
                 <h3 className="text-sm font-semibold mb-2 text-muted">Share Preview</h3>
                 <p className="text-sm text-muted mb-2">
                   <strong>Text:</strong> {getSocialShareUrls(window.location.href, token.name, token.symbol).twitter.includes("text=") ? 
                     decodeURIComponent(getSocialShareUrls(window.location.href, token.name, token.symbol).twitter.split("text=")[1].split("&")[0]) : 
                     `${token.name} ($${token.symbol}) live now on Solana.`
                   }
                 </p>
                 <p className="text-xs text-muted">
                   <strong>Link:</strong> {getSocialShareUrls(window.location.href, token.name, token.symbol).copy}
                 </p>
               </div>
             </div>

...
export async function fetchTokenStats(mint: string): Promise<TokenStats> {
  ...
  try {
    // Skip DexScreener calls if disabled (devnet mode)
        ...
        if (response.ok) {
          ...
          if (
            ...
            const solanaPairs = data.pairs.filter(
              (pair: any) => pair.chainId === "solana" && pair.liquidity?.usd
            );
    ...
    // Try to fetch holders count from Solscan (best-effort) - only if DexScreener is enabled
    if (!DEV_DISABLE_DEXSCR) {
      try {
        const holdersController = new AbortController();
        const holdersTimeoutId = setTimeout(
          () => holdersController.abort(),
          5000
        );

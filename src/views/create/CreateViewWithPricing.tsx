...
import { TokenCreationType, TOKEN_CREATION_TYPES, hasFeature } from "../../lib/tokenPricing";
...
import { TokenCreationTypeSelector } from "../../components/TokenCreationTypeSelector";
import { TokenCreationPayment } from "../../components/TokenCreationPayment";

interface CreateViewWithPricingProps {
  setOpenCreateModal: (value: boolean) => void;
}

export const CreateViewWithPricing: FC<CreateViewWithPricingProps> = ({ setOpenCreateModal }) => {
  const { connection } = useConnection();
  const { publicKey, sendTransaction } = useWallet();
  const { networkConfiguration } = useNetworkConfiguration();
  const { verifyTokenPayment, isVerifying, error: paymentError } = useTokenPayment();

  const [tokenUri, setTokenUri] = useState("");
  const [tokenMintAddress, setTokenMintAddress] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [isOnChainVerified, setIsOnChainVerified] = useState(false);
  const [selectedTokenType, setSelectedTokenType] = useState<TokenCreationType>('free');
  ...
  // Handle token creation type change
  const handleTokenTypeChange = (type: TokenCreationType) => {
    setSelectedTokenType(type);
    setPaymentTxSig(null); // Reset payment when type changes
  };

  // Handle payment completion
  const handlePaymentComplete = (txSig: string) => {
  ...
  const createToken = useCallback(
    async (token) => {
      ...
      // For Pro tokens, verify payment was completed
      if (selectedTokenType === 'pro' && !paymentTxSig) {
        notify({
          type: "error",
          message: "Please complete payment for Pro token creation",
        });
        return;
      }
      ...
      try {
        ...
        const tokenMetadata = {
          ...
          links: token.links || {},
          tokenType: selectedTokenType, // Add token creation type to metadata
          paymentTxSig: paymentTxSig, // Add payment transaction signature
        };
        ...
        if (selectedTokenType === 'pro' && paymentTxSig) {
          const paymentVerified = await verifyTokenPayment(paymentTxSig, 'pro');
          if (!paymentVerified) {
    ...
    [publicKey, connection, sendTransaction, selectedTokenType, paymentTxSig, verifyTokenPayment]
  );
              ...
              <div className="grid gap-5 lg:grid-cols-2">
                <div className="ps-4 hidden py-4 pt-10 lg:block">
                  ...
                  <div className="token-styles relative">
                    {/* Token Creation Type Selector */}
                    <div className="mt-6">
                      <TokenCreationTypeSelector
                        selectedType={selectedTokenType}
                        onTypeChange={handleTokenTypeChange}
                        disabled={isLoading}
                      />
                    </div>

                    {/* Preset Selector - only show for Pro tokens or if Free has preset feature */}
                    {(selectedTokenType === 'pro' || hasFeature(selectedTokenType, 'preset')) && (
                      <div className="mt-6">
                    ...
                    {/* Vibe Selector - only show for Pro tokens */}
                    {selectedTokenType === 'pro' && (
                      <div className="mt-6">
                        <label className="block text-muted mb-3 font-semibold">
                          Token Vibe
                        </label>
                    ...
                    <div className="text-start">
                      ...
                      <div className="text-center">
                        <button
                          ...
                          disabled={!publicKey || isLoading || (selectedTokenType === 'pro' && !paymentTxSig)}
                          className={`group mt-5 inline-flex w-full items-center justify-center rounded-lg px-6 py-2 backdrop-blur-2xl transition-all duration-500 ${
                            !publicKey || (selectedTokenType === 'pro' && !paymentTxSig)
                        ...
                        >
                          <span className="fw-bold">
                            {!publicKey
                              ...
                              : selectedTokenType === 'pro' && !paymentTxSig
                              ? "Complete Payment First"
                              : isLoading
                              ? "Creating Token..."
                              : `Create ${selectedTokenType === 'pro' ? 'Pro' : 'Free'} Token`}
                          </span>
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="my-auto text-center">
                    ...
                    <div className="mt-2 text-center">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                        ...
                        {selectedTokenType === 'pro' ? 'Pro Token' : 'Free Token'}
                      </span>
                    </div>
